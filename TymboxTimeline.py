...
class TymboxTimeline(QWidget):
    ...
    def paintEvent(self, paint_event: QPaintEvent):
        ...
        for i in range(0, self.tymbox_model.rowCount()):
            ...
            task_option.rect.adjust(option.rect.x(), option.rect.y(), option.rect.x()+option.rect.width(), 0)
    ...
    def get_task_height(self, row):
        ...
        model_duration = self.tymbox_model.duration
        model_start_time = self.tymbox_model.start_time
        height = self.height()
        pixels_minute = height / model_duration
        item_height_pixels = pixels_minute * duration
        item_y_pixels = pixels_minute * (start_time - model_start_time)
